...
(function () {
  ...
  b.fn.autoResize = function (f) {
    ...
    this.filter("textarea").each(function () {
      var l = b(this).css({ "overflow-y": "hidden", resize: "none" }),
        j = l.height(),
        i = j,
        k = (function () {
          var m = l
          ...
          return m.insertBefore(l);
        })(),
        h = function () {
          var n = l.val(),
            m;
          k.height(0).val(n).scrollTop(9999);
          m = Math.max(j, k.scrollTop());
          if (m === i) {
            return;
          }
          i = m;
      ...
      l.unbind(".resize");
      if (c()) {
        l.bind("input.resize", h);
      } else {
        if (e()) {
          l.bind("propertychanged.resize", h);
        } else {
          l.bind("keypress.resize", h);
        }
      }
      h();
    });
